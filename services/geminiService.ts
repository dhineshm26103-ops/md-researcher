...
export const generateChatResponse = async (messages: ChatMessage[], sources: SourceDocument[], query: string, language: string, userName?: string): Promise<string> => {
  const ai = await getAiClient();
  const activeSources = sources.filter(s => s.active);
  ...
  const userCtx = userName ? `The user's name is ${userName}. Address them by name when greeting.` : '';
  const hasSources = activeSources.length > 0;
  const systemInstruction = hasSources 
    ? `You are MD, an intelligent research assistant. Use the provided sources to answer precisely. ${userCtx} Language: ${language}.`
    : `You are MD, a helpful AI. No sources provided, so help user with general queries. ${userCtx} Language: ${language}.`;
...
:::EMAIL_JSON{"subject": "[MD Researcher Feedback] Category: Brief Summary", "body": "Hi Team,\\n\\nI am writing to share feedback regarding MD Researcher.\\n\\nCategory: {Category}\\nDetails: {User's specific details}\\n\\nBest regards,\\n{User Name}\\ MD Researcher @ MD Researcher"}:::
